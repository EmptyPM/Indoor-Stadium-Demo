...
export default function EditLocationPage() {
  ...
  useEffect(() => { if (loc) reset({ name: loc.name, province: loc.province ?? '' }); }, [loc, reset]);

  const onSubmit = (d: Form) => update({ id, ...d }, { onSuccess: () => router.push('/admin/locations') });
      ...
      <Card><CardContent className="p-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Button type="submit" className="w-full gap-2" disabled={isPending}>
            {isPending && <Loader2 className="w-4 h-4 animate-spin" />}{isPending ? 'Saving...' : 'Update Location'}
          </Button>
        </form>
      </CardContent></Card>
    </div>
